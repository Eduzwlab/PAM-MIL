...
class MHIM(nn.Module):
    def __init__(self, mlp_dim=512,mask_ratio=0,n_classes=2,temp_t=1.,temp_s=1.,dropout=0.25,act='relu',select_mask=True,select_inv=False,msa_fusion='vote',mask_ratio_h=0.,mrh_sche=None,mask_ratio_hr=0.,mask_ratio_l=0.,da_act='gelu',baseline='selfattn',head=8,attn_layer=0):
        ...
        ## UNI imagenet CTransPath RetCCL
        self.patch_to_emb = [nn.Linear(1024, 512)]
        #
        ## MUSK UNIv2
        # self.patch_to_emb = [nn.Linear(1536, 512)]

        # self.patch_to_emb = [nn.Linear(2048, 512)]

        # CONCH CONCHv1_5 CTransPath
        ...
        if baseline == 'selfattn':
            # self.online_encoder = SAttention(mlp_dim=mlp_dim,head=head, pos="ccpp")
            self.online_encoder = SAttention(mlp_dim=mlp_dim,head=head, pos="ppeg")
            # self.online_encoder = SAttention_v(mlp_dim=mlp_dim,head=head)
        elif baseline == 'attn':
    ...
    def forward_teacher(self,x,return_attn=False):
        ...
        if return_attn:
            x,attn = self.online_encoder(x,return_attn=False)
    ...
    def forward_test(self,x,return_attn=False,no_norm=False):
        ...
        if return_attn:
            x,a = self.online_encoder(h,return_attn=True)
        else:
            x = self.online_encoder(h)
        x = self.predictor(x)
