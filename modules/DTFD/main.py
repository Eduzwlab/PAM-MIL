...
def one_fold(params,k,train_p,train_l,test_p,test_l,val_p,val_l,epoch_step,writer):
        ...
        for ii in range(params.EPOCH):
            ...
            macc_1, auc_val, auc_value_macro, auc_value_weighted, mprec_1, precision_macro, precision_weighted, mrecal_1, recall_macro, recall_weighted, mF1_1, fscore_macro, fscore_weighted, three_scores, bag_labels, bag_predictions, y_pred, test_loss = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
            ...
            if params.always_test:
                ...
                macc_1_test, auc_val_test, auc_value_macro, auc_value_weighted, mprec_1_test, precision_macro, precision_weighted, mrecal_1_test, recall_macro, recall_weighted, mF1_1_test, fscore_macro, fscore_weighted, three_scores, bag_labels, bag_predictions, y_pred, test_loss_test = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
        ...
        tacc, tauc, auc_value_macro, auc_value_weighted, tprec, precision_macro, precision_weighted, trecal, recall_macro, recall_weighted, tf1, fscore_macro, fscore_weighted, three_scores, bag_labels, bag_predictions, y_pred, test_loss1 = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                UClassifier=attCls, mDATA_list=(SlideNames_test, FeatList_test, Label_test), criterion=ce_cri, epoch=ii,  params=params, f_log=log_file, writer=writer, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
        bag_predictions_array = np.array(bag_predictions)
        bag_predictions_list = bag_predictions_array.tolist()
        TESTsave = {
            "Name": SlideNames_test,
        ...
        testResultsPath = os.path.join(params.log_dir, params.name, 'TEST_RESULT_TILE_BASED_FOLD_{fold}.csv'.format(fold=k))
...
def test_attention_DTFD_preFeat_MultipleMean(mDATA_list, classifier, dimReduction, attention, UClassifier, epoch, criterion=None,  params=None, f_log=None, writer=None, numGroup=3, total_instance=3, distill='MaxMinS'):
        ...
        for idx in range(numIter):
            ...
            for tidx, tfeat in enumerate(batch_feat):
                ...
                for jj in range(params.num_MeanInference):
                    ...
                    # print("_________________gSlidePred____________________________")
                    # print(gSlidePred)
                    # print(gSlidePred.shape)
                    # print("_____________________________________________")

                    allSlide_pred_softmax11.append(torch.softmax(gSlidePred, dim=1))
                    allSlide_pred_softmax11 = torch.cat(allSlide_pred_softmax11, dim=0)
                    allSlide_pred_softmax11 = torch.mean(allSlide_pred_softmax11, dim=0).unsqueeze(0)
                ...
                loss1 = F.nll_loss(allSlide_pred_softmax11, tslideLabel)
    ...
    # print("_________________gPred_1____________________________")
    # print(gPred_1)
    # print(gPred_1.shape)
    # print("____________________gt_1______gt_1_____gt_1______________")
    # print(gt_1)
    # print(gt_1.shape)
    ...
    # gPred_1 = gPred_1.cpu().numpy()
    # macc_0, mprec_0, mrecal_0, mspec_0, mF1_0, auc_0,pred_0,oprob_0 = eval_metric(gPred_0, gt_0)
    # macc_1, mprec_1, mrecal_1, mspec_1, mF1_1, auc_1,oprob_1 = eval_metric(gPred_1, gt_1)


    accuracy, auc_value,auc_value_macro,auc_value_weighted, precision,precision_macro,precision_weighted, recall,recall_macro,recall_weighted, fscore,fscore_macro,fscore_weighted,three_scores,bag_labels, bag_predictions,y_pred = eval_metric(
        gt_1, bb)
    ...
    # return auc_1,mF1_1.cpu(),macc_1.cpu(),mprec_1.cpu(), mrecal_1.cpu(),pred_1.cpu(),oprob_1.cpu(),test_loss1
    return accuracy, auc_value,auc_value_macro,auc_value_weighted, precision,precision_macro,precision_weighted, recall,recall_macro,recall_weighted, fscore,fscore_macro,fscore_weighted,three_scores,bag_labels, bag_predictions,y_pred,test_loss1
...
if __name__ == "__main__":
    ...
    ####  修改'--name'和'--dataset_root', result在 /MHIM-MIL-master/DTFD/debug_log
    parser.add_argument('--name', default='TCGA', type=str)
    parser.add_argument('--project', default='mil', type=str)
    # parser.add_argument('--dataset_root', default='/mnt/volume2/Vm/Datasets/1-changhai-dataset/2-research_normalizationused_dataset/BasalLumAB/CPCGEA-SLIDES-tumortile-1to1/TCGA-CRC-DX/FEATURES-imagenet-3class', type=str, help='dataset root path')
    parser.add_argument('--dataset_root', default='/mnt/volume2/Vm/Datasets/1-changhai-dataset/2-research_normalizationused_dataset/TCGA-PRAD-oneslide/TCGA-CRC-DX/FEATURES_imagenet_498', type=str, help='dataset root path')
    parser.add_argument('--EPOCH', default=200, type=int)
    parser.add_argument('--cv_fold', default=5, type=int, help='Number of cross validation fold [3]')
