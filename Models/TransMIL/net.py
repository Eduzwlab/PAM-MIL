...
class TransMIL(nn.Module):
    def __init__(self, n_classes, input_size, confounder_path=None):
        ...
        self._fc3 = nn.Sequential(nn.Linear(512, input_size), nn.ReLU())
        self.norm2 = nn.LayerNorm(input_size)

        self._fc2 = nn.Linear(input_size, self.n_classes)
        self._fc4 = nn.Linear(1280, self.n_classes)
        ...
        if confounder_path:
            ...
            self.confounder_W_k = nn.Linear(input_size, joint_space_dim)
    ...
    def forward(self, feats):
        ...
        # torch.Size([1, 12545, 512])

        # h_768 = self.layer3(h) #[B, N, 512]
        ...
        if self.confounder_path:
            ...
            if 'nn' not in self.confounder_path[0]: # un normalized 
                h1 = self.norm(h)[:,0]
                device = h1.device
                bag_q = self.confounder_W_q(h1)
                conf_k = self.confounder_W_k(self.confounder_feat)
                A1 = torch.mm(conf_k, bag_q.transpose(0, 1))
                A2 = F.softmax( A1 / torch.sqrt(torch.tensor(conf_k.shape[1], dtype=torch.float32, device=device)), 0) # normalize attention scores, A in shape N x C,
                conf_feats = torch.mm(A2.transpose(0, 1), self.confounder_feat) # compute bag representation, B in shape C x V
                h = torch.cat((h1,conf_feats),dim=1)
        ...
        Bag_feature = self._fc3(h) #[B, n_classes]
        Bag_feature = self.norm2(Bag_feature)
        if not self.confounder_path:
            logits = self._fc2(Bag_feature) #[B, n_classes]
        else:
            logits = self._fc4(h)

        Y_hat = torch.argmax(logits, dim=1)
        Y_prob = F.softmax(logits, dim = 1)
        results_dict = {'logits': logits, 'Y_prob': Y_prob, 'Y_hat': Y_hat, "Bag_feature":Bag_feature, "A":A, 'h_not_norm':h_not_norm}
